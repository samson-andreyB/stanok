...
function copyNodeBinary() {
  ...
  const runtimeDestPath = path.join(runtimeRoot, runtimeNodeRelPath());
  ensureDir(path.dirname(runtimeDestPath));
  fs.rmSync(runtimeDestPath, { force: true });
  fs.copyFileSync(sourceNodeBin, runtimeDestPath);
  if (process.platform !== 'win32') {
    fs.chmodSync(runtimeDestPath, 0o755);
  }
  return { triple, destName, destPath, runtimeDestPath };
}
...
function main() {
  const { triple, destPath, runtimeDestPath } = copyNodeBinary();
  let mode = 'skipped';
  ...
  console.log(`Runtime Node prepared: ${runtimeDestPath}`);
