...
#[test]
fn pretransform_stress_produces_stable_output_across_repeated_compiles() {
    let cwd = temp_dir("stress-stability");
    let src = cwd.join("assets/css/src");
    let modules = src.join("modules");
    let img_dest = cwd.join("assets/img/dest");
    std::fs::create_dir_all(&modules).expect("src modules dir should exist");
    ...
    let baseline = compile_one_entry(&cwd);
    for _ in 0..24 {
        let css = compile_one_entry(&cwd);
        assert_eq!(
            css, baseline,
            "repeated pretransform/compile must produce byte-identical CSS output"
        );
    }
}
