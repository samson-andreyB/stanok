...
fn start_project_watch(
    ...
    let mut guard = state
        .handle
        .lock()
        .map_err(|_| "Ошибка блокировки watcher".to_string())?;
...
fn run_project_watch_event_loop(
    ...
    let mut watcher = match RecommendedWatcher::new(
        ...
        Err(error) => {
            ...
            run_project_watch_polling_loop(
                app_handle,
                stop_worker,
                projects_path,
                project_name,
...
fn start_branch_watch(
    ...
    let mut guard = state
        .handle
        .lock()
        .map_err(|_| "Ошибка блокировки branch watcher".to_string())?;
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            loop {
                ...
                let cpu_percent = if has_prev {
                    let delta_process =
                        snapshot.process_jiffies.saturating_sub(prev_process) as f64;
...
fn run_node_script(
    ...
    for attempt in 0..=WORKER_RETRIES {
        match run_node_script_via_rust_worker(state, runtime, script_name, payload) {
...
fn run_node_script_via_rust_worker(
    _state: &BuildOrchestratorState,
    runtime: &RuntimePathsState,
    script_name: &str,
    payload: &BuildPayload,
) -> Result<String, String> {
    ...
    cmd.current_dir(strip_windows_verbatim_prefix(
        runtime.workspace_root.clone(),
    ));
    if let Some(node_modules_dir) = &runtime.node_modules_dir {
        cmd.env(
            "NODE_PATH",
            strip_windows_verbatim_prefix(node_modules_dir.clone()),
        );
    }
...
fn build_config_from_project_data(project_data: Option<&Value>) -> BuildConfig {
    ...
    let path_str =
        |key: &str| -> Option<&str> { path.and_then(|p| p.get(key)).and_then(Value::as_str) };
...
fn scan_projects(projects_path: &str) -> Result<ProjectsResponse, String> {
    ...
    for entry in entries {
        ...
        for file_name in ["verstak.json", "polki.json"] {
            ...
            if manifest.exists() {
                let project_type = if file_name == "verstak.json" {
                    "verstak"
                } else {
                    "polki"
                };
...
fn projects_cache_file(projects_path: &str) -> Option<PathBuf> {
    ...
    Some(
        cache_root
            .join("stanok")
            .join(format!("projects-{:x}.json", hash)),
    )
}
...
fn run_build_orchestrated(
...

    {
        let mut guard = state
            .entries
            .lock()
            .map_err(|_| "Ошибка блокировки очереди сборки".to_string())?;
                ...
                let out = match resolve_styles_engine(&styles_payload) {
                    ...
                    StylesEngine::Rust => match run_styles_rust(&styles_payload) {
                        ...
                        Err(err) => {
                            ...
                            outputs.push(format!(
                                "Rust style pipeline failed, fallback to legacy: {err}"
                            ));
                            run_node_script(state, runtime, "build-css.mjs", &styles_payload)?
                        }
                    },
...
fn build_path_to_projects_root(nest: &str) -> String {
    ...
    let level = n.split(['/', '\\']).filter(|part| !part.is_empty()).count();
    "../".repeat(level)
}

fn build_runtime_paths(payload: &BuildPayload) -> BuildRuntimePaths {
    let project_dir = resolve_project_dir(
        &payload.projects_path,
...
fn run_images_rust(payload: &BuildPayload) -> Result<String, String> {
    ...
    fs::create_dir_all(&dest_dir)
        .map_err(|e| format!("Ошибка создания папки {}: {}", dest_dir.display(), e))?;
    ...
    while let Some(current) = stack.pop() {
        let entries = fs::read_dir(&current)
            .map_err(|e| format!("Ошибка чтения папки {}: {}", current.display(), e))?;
        for entry in entries {
            let entry = entry.map_err(|e| e.to_string())?;
...
fn run_styles_rust(payload: &BuildPayload) -> Result<String, String> {
    ...
    let result =
        style_pipeline::compile(req).map_err(|e| format_rust_style_error(&e.to_string()))?;
    let pre_profiles: Vec<String> = result
        .diagnostics
        .iter()
        .filter(|d| d.code == "PREPROCESS_PROFILE")
    ...
    if !pre_profiles.is_empty() {
        msg.push_str("\npre-transform profile:");
        for line in pre_profiles {
            msg.push_str("\n- ");
            msg.push_str(&line);
        }
    }
...
fn parse_project_manifest(
    ...
    let mut data: Value =
        serde_json::from_str(&source).map_err(|e| format!("{}: {}", manifest_path.display(), e))?;
    ...
    if let Some(nested) = data.get("nest").and_then(Value::as_str) {
        ...
        let nested_path = projects_root
            .join(pure_name)
            .join(nested)
            .join(manifest_name);
        let nested_source = fs::read_to_string(&nested_path).map_err(|e| e.to_string())?;
        data = serde_json::from_str(&nested_source)
            .map_err(|e| format!("{}: {}", nested_path.display(), e))?;
    }
...
fn make_project_item(
    ...
    let short_name = data.get("name").and_then(Value::as_str).unwrap_or("main");
...
fn normalize_project_data(data: Value) -> Option<Value> {
    ...
    let path_value = obj.entry("path").or_insert_with(|| {
        serde_json::json!({
          "root": root_default,
          "data": data_default
        })
    ...
    if let Some(path_obj) = path_value.as_object_mut() {
        ...
        if let Some(Value::String(data_segment)) = path_obj.get("data").cloned() {
            ...
            path_obj.insert("data".to_string(), Value::String(joined.replace('\\', "/")));
        }
    }
...
fn detect_node_version(node_bin: &Path) -> String {
    let output = command_with_platform_defaults(strip_windows_verbatim_prefix(node_bin))
        .arg("-v")
        .output();
    match output {
...
fn resolve_runtime_paths(app: &tauri::AppHandle) -> RuntimePathsState {
    let dev_workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
...
fn enrich_runtime_module_error(
    ...
    let mut probe_lines = vec![format!(
        "script: {} [{}]",
        script_path.display(),
        if script_path.exists() {
            "exists"
...
mod tests {
    ...
    fn format_rust_style_error_keeps_plain_message_for_other_errors() {
        ...
        let out = format_rust_style_error(raw);
        assert!(
            out.contains("Rust style pipeline failed: Failed to resolve @import 'lib/_helpers'")
        );
        assert!(!out.contains("legacy nested-синтаксис"));
    }
    ...
    fn watch_snapshot_detects_css_and_img_changes() {
        ...
        let first =
            compute_project_watch_snapshot(root.to_string_lossy().as_ref(), "demo/main", &cfg)
